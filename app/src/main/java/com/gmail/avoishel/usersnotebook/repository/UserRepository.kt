...
// todo ох.. 1) я просил обработать ошибки... хде?  // ---> data.retrofit.ApiClient  -  или я что то не так понял?
//  2) а почему конструктор хилта пустой? я в нем фижу синглинстанс ретрофита  // ---> сделал, лежит тут: di.ApiModels
//  нужно почитать саму суть хилта, зачем он вообще нам
//  3) почитать про солид, в данном случае над любым репозиторием должен быть интерфейс для каждой функции,
//  которую ты хочешь здесь увидеть будет для тебя как клад https://github.com/JohnnySC/Lectures  // ---> не понял что нужно сделать (
